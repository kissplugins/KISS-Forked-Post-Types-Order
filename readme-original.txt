...
= 2.3.9 =
 - Fixed category filter visibility issue - filter now only shows when hierarchical taxonomies with terms are available.
 - Improved category filter UI styling with better visual separation and spacing.
 - Added debugging comments to help identify when category filter is not displayed.
 - Synchronized version numbers across all plugin files.

= 2.3.8 =
 - Added category filtering feature to the Post Ordering screen.
 - Users can now filter posts by category/taxonomy using a dropdown selector.
 - Implemented AJAX-based filtering for seamless user experience without page reloads.
 - Enhanced order preservation logic to maintain global post order when filtering by categories.
 - Added "Clear Filter" functionality to easily return to viewing all posts.
